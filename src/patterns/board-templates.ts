/**
 * Built-in board paper patterns for runtime resolution (no DB seed required).
 * Ground-truth structures mirror prisma/seed-patterns.ts verified templates.
 */

import {
  PatternDataDef,
  PatternSectionDef,
  buildPectaSciencePattern,
...
export function normalizeTemplateSubject(subject: string): string {
...
function sscMathCsPattern(subject: string, classLevel: string): PatternDataDef {
...
function sscEnglishPattern(classLevel: string): PatternDataDef {
  return {
...
function sscUrduPattern(classLevel: string): PatternDataDef {
  return {
...
function sscIslamiatPattern(classLevel: string): PatternDataDef {
  return {
...
function sscPakStudiesPattern(classLevel: string): PatternDataDef {
  return {
...
function hsscSciencePattern(subject: string, classLevel: string): PatternDataDef {
...
function hsscMathPattern(classLevel: string): PatternDataDef {
  return {
...
function hsscCsPattern(classLevel: string): PatternDataDef {
  return {
...
/** Build a built-in template from code when DB template is unavailable. */
export function buildBoardTemplateFromCode(
  board: string | null,
  country: string | null,
  subject: string,
  ...
  if (!board || country !== 'Pakistan' || !classLevel) return null;
  ...
  if (board === 'BISE Punjab' && syllabusVariant === 'pecta' && ['9', '10'].includes(cls)) {
    if (isPectaScienceSubject(normSubject)) {
      return buildPectaSciencePattern(normSubject, cls);
    }
  }
  ...
  if (['9', '10'].includes(cls)) {
    ...
    if (normSubject === 'English') return sscEnglishPattern(cls);
    if (normSubject === 'Urdu') return sscUrduPattern(cls);
    if (normSubject === 'Islamiat') return sscIslamiatPattern(cls);
    if (normSubject === 'Pakistan Studies') return sscPakStudiesPattern(cls);
    if (isPectaScienceSubject(normSubject)) {
      return buildPectaSciencePattern(normSubject, cls);
    }
  }
  ...
  if (['11', '12'].includes(cls)) {
    ...
    if (normSubject === 'Mathematics') return hsscMathPattern(cls);
    if (normSubject === 'Computer Science') return hsscCsPattern(cls);
...
export function builtInBoardPatternId(
  board: string,
  subject: string,
  classLevel: string,
): string {
...
/** Legacy PECTA science IDs (backward compatible). */
export function legacyPectaPatternId(subject: string, classLevel: string): string {
  const normSubject = normalizeTemplateSubject(subject).toLowerCase().replace(/\s+/g, '_');
  return `builtin:pecta:${normSubject}:${classLevel}`;
}

export function patternIdMatchesBuiltIn(
  patternId: string,
  board: string,
  subject: string,
  classLevel: string,
  ...
  const cls = classLevel.replace(/\D/g, '') || classLevel;
  return (
    patternId === builtInBoardPatternId(board, normSubject, cls) ||
    patternId === legacyPectaPatternId(normSubject, cls)
  );
}

export function subjectsWithBuiltInTemplates(board: string, classLevel: string): string[] {
  const cls = classLevel.replace(/\D/g, '') || classLevel;
  if (board !== 'BISE Punjab') return [];
  if (['9', '10'].includes(cls)) {
    return [
      'Physics',
      'Chemistry',
