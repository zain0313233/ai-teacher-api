...
interface PatternSection {
  name: string;
  questionType: 'MCQ' | 'Short Answer' | 'Long Answer' | 'Case Study' | 'Practical' | 'Essay';
...
"20 MCQs only" → {"board": null, "country": null, "subject": "Mathematics", "class": null, "year": "2026", "needsWebSearch": false}`;
  ...
  private async searchWebForPattern(context: DetectedContext): Promise<string> {
    // Build country-specific search query to avoid ambiguity
    let searchQuery: string;
    
    if (context.board === 'BISE Punjab' && context.country === 'Pakistan') {
      // Very specific query for Pakistani BISE to avoid Indian PSEB results
...
- NO markdown, NO explanation, ONLY JSON`;
  ...
  private validateAndCorrectPattern(
    ...
    // Known correct patterns — ground truth
    const KNOWN_PATTERNS: Record<string, Partial<PatternData>> = {
    ...
    if (knownPattern) {
      ...
      // Check if generated pattern is wrong
      ...
      if (isWrongMarks || isWrongDuration || isZeroDuration) {
        ...
        return {
          ...
          sections: (knownPattern.sections ?? patternData.sections) as PatternSection[],
        };
      }
