...
exports.getBoardBenchmarkTarget = getBoardBenchmarkTarget;
exports.classifyStrength = classifyStrength;
exports.buildBenchmarkMessage = buildBenchmarkMessage;
exports.aggregateTopicWeakness = aggregateTopicWeakness;
function getBoardBenchmarkTarget(board) {
    const key = (board || '').toLowerCase();
    if (key.includes('cambridge') || key.includes('o level'))
        return 75;
    if (key.includes('federal'))
        return 68;
    if (key.includes('sindh'))
        return 67;
    if (key.includes('kpk'))
        return 66;
    return 70;
}
function classifyStrength(avgScore) {
    if (avgScore >= 80)
        return 'strong';
    if (avgScore >= 60)
        return 'moderate';
...
function buildBenchmarkMessage(classAvg, benchmark, subject) {
    if (classAvg == null) {
...
function aggregateTopicWeakness(answers, questionMap, limit = 8) {
    const stats = new Map();
    for (const ans of answers) {
        ...
        const topic = q?.topicTag?.trim() ||
            ...
            (session.chapterStart
                ? `Chapter ${session.chapterStart}${session.chapterEnd && session.chapterEnd !== session.chapterStart
                    ? `–${session.chapterEnd}`
                    : ''}`
                : 'General');
        const key = `${session.subject}::${topic}`;
        const chapterLabel = session.chapterStart && session.chapterEnd
