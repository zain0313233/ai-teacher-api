...
exports.isPectaScienceSubject = isPectaScienceSubject;
exports.detectSyllabusVariant = detectSyllabusVariant;
exports.buildPectaSciencePattern = buildPectaSciencePattern;
exports.buildPectaTemplateIfApplicable = buildPectaTemplateIfApplicable;
const PECTA_SCIENCE_SUBJECTS = ['Physics', 'Chemistry', 'Biology'];
function isPectaScienceSubject(subject) {
    const s = subject.trim().toLowerCase();
    return PECTA_SCIENCE_SUBJECTS.some((p) => s.includes(p.toLowerCase()));
}
function detectSyllabusVariant(userPrompt) {
    ...
    if (legacyScore > pectaScore)
        return 'legacy';
    if (pectaScore > 0)
        return 'pecta';
    if ((p.includes('punjab') || p.includes('bise') || p.includes('lahore')) &&
...
function buildPectaSciencePattern(subject, classLevel) {
    ...
    const sections = [
        {
            ...
            marksPerQuestion: 1,
            notes: 'PECTA Smart Syllabus: 12 MCQs × 1 mark. Typical split — 2 MCQs each from Ch.3, 4, 6; 1 MCQ each from Ch.1, 2, 5, 7, 8, 9. Separate objective sheet (~15 min).',
        },
        {
        ...
        },
        {
            ...
            questionsToAttempt: 5,
            marksPerQuestion: 2,
            notes: 'Chapters 7, 8, 9 — 8 parts (Ch7: 3, Ch8: 3, Ch9: 2). Attempt any 5 parts × 2 marks = 10 marks. Ch.9 often MCQ/short only.',
        },
        {
            ...
            marksPerQuestion: 9,
            notes: 'Q.5: Ch1+2 (Physical quantities & Kinematics). Q.6: Ch4+6 or Ch3+5 (board pairing). Q.7: Ch7+8. Each long Q = 9 marks (often 5+4 theory+numerical). Attempt any 2.',
        },
    ];
    return {
...
function buildPectaTemplateIfApplicable(board, country, subject, classLevel, syllabusVariant) {
    if (syllabusVariant !== 'pecta')
        return null;
    if (board !== 'BISE Punjab' || country !== 'Pakistan')
        return null;
    if (!classLevel || !['9', '10'].includes(classLevel))
        return null;
    if (!isPectaScienceSubject(subject))
        return null;
    return buildPectaSciencePattern(subject, classLevel);
}
//# sourceMappingURL=pecta-templates.js.map