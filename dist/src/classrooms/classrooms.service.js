...
const exam_genie_service_1 = require("../exam-genie/exam-genie.service");
...
let ClassroomsService = class ClassroomsService {
    ...
    constructor(prisma, examGenieService, patternsService, notificationsService, questionBanksService) {
        this.prisma = prisma;
    ...
    async uniqueJoinCode() {
        ...
        throw new common_1.BadRequestException('Could not generate join code. Try again.');
    }
    ...
    isAssignmentPublished(publishAt) {
        if (!publishAt)
            return true;
    ...
    validateAssignmentSchedule(publishAt, dueAt) {
        if (publishAt && dueAt && publishAt.getTime() >= dueAt.getTime()) {
            throw new common_1.BadRequestException('Opens at must be before the due date');
        }
    }
    async notifyAssignmentIfPublished(assignment, classroom) {
        if (!this.isAssignmentPublished(assignment.publishAt))
            return;
        if (assignment.publishNotifiedAt)
            return;
    ...
    assignmentStatusSortOrder(status) {
        const order = {
            overdue: 0,
            due_soon: 1,
            pending: 2,
    ...
    publishedAssignmentFilter(now = new Date()) {
        return {
            OR: [{ publishAt: null }, { publishAt: { lte: now } }],
        };
    }
    resolveProctoringEnabled(assignmentMode, enabled) {
        return assignmentMode === 'timed' && Boolean(enabled);
    }
    async assertStudentEnrollment(studentId, classroomId) {
    ...
    async createClassroom(teacherId, dto) {
    ...
    async getTeacherClassroom(teacherId, classroomId) {
    ...
    async joinClassroom(studentId, dto) {
        const classroom = await this.prisma.classroom.findFirst({
    ...
    async listStudentClassrooms(studentId) {
        ...
        const classrooms = await Promise.all(enrollments.map(async (e) => {
    ...
    async listStudentAssignments(studentId) {
        ...
        const mapped = assignments
            ...
            const { status, isOverdue, isPublished } = this.resolveAssignmentStatus(submitted, a.dueAt, a.publishAt);
            ...
            .sort((a, b) => {
            const priority = this.assignmentStatusSortOrder(a.status) -
    ...
    async getStudentClassroom(studentId, classroomId) {
    ...
    async shareMaterial(teacherId, classroomId, dto) {
        await this.assertTeacherClassroom(teacherId, classroomId);
        if (!dto.documentId && !dto.pastPaperId) {
            throw new common_1.BadRequestException('documentId or pastPaperId is required');
        }
        if (dto.documentId) {
    ...
    computeTimedWindow(assignment, startedAt) {
    ...
    assertAssignmentOpen(assignment) {
        ...
        if (assignment.dueAt && new Date() > assignment.dueAt) {
            throw new common_1.BadRequestException('Assignment deadline has passed');
        }
    }
    compositionFromPatternSections(sections) {
    ...
    async createAssignment(teacherId, classroomId, dto) {
        ...
        if (source === 'bank' && dto.patternId) {
            throw new common_1.BadRequestException('Question bank assignments cannot use a paper pattern');
        }
        if (source === 'bank') {
            if (assignmentMode === 'timed' && !durationMinutes) {
                throw new common_1.BadRequestException('durationMinutes is required for timed assignments');
            }
            const bankItems = await this.questionBanksService.resolveItemsForAssignment(teacherId, classroomId, dto.bankItemIds ?? []);
        ...
        if (dto.patternId) {
            ...
            const pattern = await this.patternsService.resolvePatternForAssignment(teacherId, dto.patternId, dto.subject, {
                classGrade: classroom?.classGrade ?? undefined,
                board: classroom?.board ?? undefined,
            });
            patternSections = pattern.sections || [];
    ...
    async duplicateAssignment(teacherId, classroomId, assignmentId, dto) {
        await this.assertTeacherClassroom(teacherId, classroomId);
        ...
        const newSession = await this.examGenieService.cloneClassQuizSession(source.quizSessionId, teacherId);
        ...
        const dueAt = dto.dueAt ? new Date(dto.dueAt) : null;
        this.validateAssignmentSchedule(publishAt, dueAt);
        const proctoringEnabled = this.resolveProctoringEnabled(assignmentMode, dto.proctoringEnabled ?? source.proctoringEnabled);
        const assignment = await this.prisma.classAssignment.create({
            data: {
                classroomId,
                teacherId,
    ...
    async getAssignmentQuiz(studentId, assignmentId) {
        ...
        const result = await this.examGenieService.getQuizSessionForTaking(assignment.quizSessionId, studentId, assignment.allowReviewAfterSubmit);
        let timing = null;
        if (assignment.assignmentMode === 'timed' && !result.alreadySubmitted) {
            const attempt = await this.examGenieService.startOrGetClassQuizAttempt(assignment.quizSessionId, studentId);
        ...
        return {
            ...
            timing,
            timeExpired: assignment.assignmentMode === 'timed' &&
                timing?.remainingSeconds === 0 &&
                !result.alreadySubmitted,
        };
    }
    async submitAssignment(studentId, assignmentId, dto) {
        const assignment = await this.prisma.classAssignment.findUnique({
            where: { id: assignmentId },
        });
        if (!assignment || assignment.status !== 'active') {
        ...
        if (assignment.assignmentMode === 'timed') {
            ...
            if (!inProgress) {
                throw new common_1.BadRequestException('Start the timed quiz before submitting');
            }
        ...
        return this.examGenieService.submitQuizAnswers(studentId, assignment.quizSessionId, dto, {
    ...
    async getClassroomReport(teacherId, classroomId) {
        await this.assertTeacherClassroom(teacherId, classroomId);
        const enrollments = await this.prisma.classEnrollment.findMany({
        ...
        const reports = assignments.map((a) => {
            const submittedStudentIds = new Set(a.quizSession.attempts.map((att) => att.userId));
            const studentResults = enrollments.map((e) => {
                const attempt = a.quizSession.attempts.find((att) => att.userId === e.studentId);
                return {
            ...
            return {
                ...
                averageScore: a.quizSession.attempts.length > 0
                    ? Math.round((a.quizSession.attempts.reduce((sum, att) => sum + (att.percentage || 0), 0) /
    ...
    async exportClassroomReportCsv(teacherId, classroomId) {
        ...
        for (const r of report.reports) {
            for (const s of r.studentResults) {
                rows.push([
                    ...
                    this.escapeCsv(s.completedAt ? new Date(s.completedAt).toISOString() : ''),
                ].join(','));
            }
        }
        return `\uFEFF${rows.join('\n')}`;
...
exports.ClassroomsService = ClassroomsService = __decorate([
    ...
    __metadata("design:paramtypes", [prisma_service_1.PrismaService,
        exam_genie_service_1.ExamGenieService,
